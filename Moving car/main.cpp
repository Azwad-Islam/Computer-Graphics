...
float angle = 0.0f;
float carpos = -4.27312f; // Initial car position
...
void display()
{
	...
	glPushMatrix();
	glTranslatef(carpos, 0.0, 0.0); // Move the car left to right
	...
	glPushMatrix();
	glTranslatef(carpos - 9.59911, 0.0, 0.0);//2nd wheel posistion
	//back wheel
	glRotatef(angle, 0.0f, 0.0f, 1.0f);
	Wheel();
	glPopMatrix();
...
void update(int value)
{
	...
	carpos += 0.2f; //car speed

	if (carpos > 30.0f) {
		carpos = -15.0f; // Reset initial car position when out of sight
	}
	glutPostRedisplay();
